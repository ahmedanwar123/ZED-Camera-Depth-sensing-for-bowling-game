...
init_params.depth_mode = sl.DEPTH_MODE.ULTRA  # Choose the desired depth mode
zed = sl.Camera()
...
with mp_pose.Pose(static_image_mode=True, min_detection_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        if zed.grab(runtime_parameters) == sl.ERROR_CODE.SUCCESS:
            zed.retrieve_image(image, sl.VIEW.LEFT)  # Retrieve the left image from the camera
            zed.retrieve_image(depth, sl.VIEW.DEPTH)  # Retrieve the depth map from the camera
